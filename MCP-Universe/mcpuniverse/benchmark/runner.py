        ...
        for benchmark in self._benchmark_configs:
            ...
            for idx, task_path in enumerate(benchmark.tasks):
                async with AsyncExitStack():
                    ...
                    try:
                        os.environ["TRACE_ID"] = getattr(tracer, "trace_id", "") or os.environ.get("TRACE_ID", "")
                    except Exception:
                         pass

                    try:
                        setattr(self._context, "trace_id", getattr(tracer, "trace_id", ""))
                    except Exception:
                        pass

                    try:
                        from mcpuniverse.tracer import collectors as _trace_collectors_mod
                        if not hasattr(_trace_collectors_mod, "get"):
                            def _get(_tid: str):
                                try:
                                    return trace_collector.get(_tid) if trace_collector else []
                                except Exception:
                                    return []
                                setattr(_trace_collectors_mod, "get", _get)
                        if not hasattr(_trace_collectors_mod, "default"):
                            setattr(_trace_collectors_mod, "default", trace_collector)
                        else:
                            try:
                                setattr(_trace_collectors_mod, "default", trace_collector)
                            except Exception:
