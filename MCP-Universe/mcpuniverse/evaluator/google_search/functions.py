...
def google_search__call_gpt(
    ...
    # 读取 DashScope 的 OpenAI 兼容变量
    api_key = context.get_env("OPENAI_API_KEY") or os.getenv("OPENAI_API_KEY")
    base_url = context.get_env("OPENAI_BASE_URL") or os.getenv("OPENAI_BASE_URL")

    # 允许通过 kwargs 或环境变量覆盖评测模型
    judge_model = (
        model
        or kwargs.get("judge_model")
        or context.get_env("JUDGE_MODEL")
        or os.getenv("JUDGE_MODEL")
        or "qwen/qwen3-4b:free"   # 默认用免费额度多的
    )
    ...
    while attempt > 0:
        try:
            resp = client.chat.completions.create(
                model=judge_model,                         # ← 不再写死 gpt-4.1
                messages=[
                    {"role": "system", "content": "You are a careful grading judge. Output EXACTLY with the keys required."},
