...
class MCPManager(metaclass=AutodocABCMeta):
    ...
    async def build_client(
        ...
        """
        assert transport in ["stdio", "sse", "streamable_http", "auto"], \
            "Transport type should be `stdio`, `sse`, `streamable_http`, or `auto`"
        assert server_name in self._server_configs, f"Unknown server: {server_name}"
        # print("-------------")
        # print(self._server_configs)
        # print("-------------")
        server_config = self._server_configs[server_name]

        effective_transport = transport
        if transport == "auto":
            if server_config.streamable_http.url:
                effective_transport = "streamable_http"
            else:
                effective_transport = "stdio"

        if effective_transport == "stdio" and not server_config.stdio.command and server_config.streamable_http.url:
            effective_transport = "streamable_http"
        ...
        if effective_transport == "streamable_http" and server_config.streamable_http.list_unspecified_params():
            raise RuntimeError(
                f"Server {server_name} has unspecified parameters: "
                f"{server_config.streamable_http.list_unspecified_params()}"
            )
        ...
        if effective_transport == "stdio":
            ...
            http_config = server_config.streamable_http
            if not http_config.url:
                raise RuntimeError(f"Server {server_name} does not define a streamable HTTP endpoint.")
