...
class BaseAgent(Executor, ExportConfigMixin, metaclass=ComponentABCMeta):
    ...
    async def reload_tools(self, server_name: str):
        ...
        Used for dynamic tool loading (e.g., Pipedream's app discovery).
        
        Args:
            server_name (str): The name of the MCP server to reload tools from
        """
        if server_name not in self._mcp_clients:
            self._logger.warning(f"Server {server_name} not found in clients")
            return
        
        self._logger.info(f"🔄 Reloading tools for server: {server_name}")
        client = self._mcp_clients[server_name]
        tools = await client.list_tools()
        
        # Update the tools cache
        self._tools[server_name] = tools
        self._logger.info(f"✅ Reloaded {len(tools)} tools for {server_name}")
        for tool in tools:
            self._logger.info(f"  📋 Tool: {tool.name}")
